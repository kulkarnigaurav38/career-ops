...
<!-- ============================================================
     ...
     2. DOCX-Master in templates/cv/ anlegen (Lebenslauf + Anschreiben, DE + EN)
...
| Datei | Pfad | Wann |
|-------|------|------|
| DOCX-Master | `templates/cv/Lebenslauf_Gaurav_Kulkarni_DE.docx` (DE) / `templates/cv/CV_Gaurav_Kulkarni_EN.docx` (EN) | IMMER — mit `python-docx` lesen |
...
**REGEL: Niemals Kennzahlen aus Proof Points hartcodieren.** Lies sie zur Bewertungszeit aus den DOCX-Mastern (via `python-docx`) und `article-digest.md`. Es gibt keine Markdown-CV-Schicht mehr.
**REGEL: Bei Kennzahlen zu Artikeln/Projekten hat `article-digest.md` Vorrang vor den DOCX-Bullets** (in den DOCX-Bullets können ältere Zahlen stehen).
...
> **Konkrete Kennzahlen: zur Bewertungszeit aus den DOCX-Mastern (via `python-docx`) und `article-digest.md` lesen. NIEMALS hier hartcodieren.**

| Wenn die Rolle ist... | Beim Kandidaten betonen... | Quellen für Proof Points |
|-----------------------|----------------------------|--------------------------|
| Platform / LLMOps | Production-Erfahrung, Observability, Evals, Closed-Loop | article-digest.md + DOCX-Master |
| Agentic / Automation | Multi-Agent-Orchestrierung, HITL, Zuverlässigkeit, Kosten | article-digest.md + DOCX-Master |
| Technical AI PM | Product Discovery, PRDs, Metriken, Stakeholder-Management | DOCX-Master + article-digest.md |
| Solutions Architect | Systemdesign, Integrationen, Enterprise-tauglich | article-digest.md + DOCX-Master |
| Forward Deployed Engineer | Schnelle Lieferung, kundennah, Prototyp bis Produktion | DOCX-Master + article-digest.md |
| AI Transformation Lead | Change Management, Team-Enablement, Adoption | DOCX-Master + article-digest.md |
...
2. Die DOCX-Master in `templates/cv/` direkt verändern — nur job-spezifische Kopien in `output/` erzeugen
...
1. Die DOCX-Master (via `python-docx`) und `article-digest.md` (falls vorhanden) lesen, bevor irgendeine Stellenanzeige bewertet wird
...
| Read | DOCX-Master in `templates/cv/` (via `python-docx`), `article-digest.md` |
