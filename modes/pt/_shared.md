...
|---------|---------|--------|
| DOCX masters | `templates/cv/CV_Gaurav_Kulkarni_EN.docx` (EN) / `templates/cv/Lebenslauf_Gaurav_Kulkarni_DE.docx` (DE) | SEMPRE — ler via `python-docx` |
...
**REGRA: NUNCA fazer hardcode de metricas de proof points.** Leia-as dos masters DOCX (via `python-docx`) e `article-digest.md` no momento da avaliacao. Nao existe mais camada de CV em markdown.
**REGRA: Para metricas de artigos/projetos, `article-digest.md` tem prioridade sobre os bullets do DOCX** (os bullets podem conter numeros desatualizados).
...
> **Metricas concretas: ler dos masters DOCX (via `python-docx`) e `article-digest.md` no momento da avaliacao. NUNCA fazer hardcode aqui.**

| Se a vaga e... | Enfatizar no candidato... | Fontes de Proof Points |
|----------------|--------------------------|------------------------|
| Platform / LLMOps | Experiencia em producao, Observability, Evals, Closed-Loop | article-digest.md + DOCX masters |
| Agentic / Automation | Orquestracao multi-agent, HITL, Confiabilidade, Custos | article-digest.md + DOCX masters |
| Technical AI PM | Product Discovery, PRDs, Metricas, Gestao de stakeholders | DOCX masters + article-digest.md |
| Solutions Architect | Design de sistemas, Integracoes, Enterprise-ready | article-digest.md + DOCX masters |
| Forward Deployed Engineer | Entrega rapida, proximo do cliente, Prototipo a producao | DOCX masters + article-digest.md |
| AI Transformation Lead | Gestao de mudanca, Enablement de equipe, Adocao | DOCX masters + article-digest.md |
...
2. Modificar os masters DOCX em `templates/cv/` diretamente — apenas criar copias por vaga em `output/`
...
1. Ler os masters DOCX (via `python-docx`), `_profile.md` e `article-digest.md` (se existir) antes de avaliar qualquer vaga
...
| Read | masters DOCX em `templates/cv/` (via `python-docx`), `_profile.md`, `article-digest.md` |
