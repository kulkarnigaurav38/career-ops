...
4. **CV** — DOCX masters in `templates/cv/` (read via `python-docx`) + `article-digest.md` — for proof points
...
Questions the interviewer will probably ask about gaps, transitions, or unusual elements in the candidate's background. Read `_profile.md` and the DOCX CV masters (`templates/cv/`, via `python-docx`) to identify what might raise questions.
...
For each gap, suggest: "You need a story about {topic}. Consider: {specific experience from the DOCX CV master that could become a STAR+R story}."
